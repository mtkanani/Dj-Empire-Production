...
import { Search, Bell, LogOut, User, Settings, Menu, Shield } from 'lucide-react';
import { C } from '../../constants/theme.js';
import { AdminBreadcrumbs } from './AdminBreadcrumbs.jsx';
import { useAuth } from '../../hooks/useAuth.js';
import { useToast } from '../../hooks/useToast.js';

export const AdminTopbar = ({ onMobileToggle }) => {
  ...
  const handleLogout = async () => {
    ...
    await logout();
    showToast('Super Admin logged out successfully', 'info');
    navigate('/login');
  };

  const adminName = `${user?.firstName || 'Super'} ${user?.lastName || 'Admin'}`;
  const initials = `${(user?.firstName || 'S')[0]}${(user?.lastName || 'A')[0]}`;
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          >
            ...
            <div style={{ textAlign: 'left', display: 'none' }} className="admin-profile-name">
              <span style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: C.text, fontFamily: 'Space Grotesk, sans-serif' }}>
                {adminName}
              </span>
              <span style={{ fontSize: '11px', color: C.gold }}>SUPER ADMIN</span>
            </div>
          </button>
          ...
          {dropdownOpen && (
            ...
            >
              <div style={{ padding: '10px 12px', borderBottom: `1px solid ${C.border}`, marginBottom: '4px' }}>
                ...
                <span style={{ color: C.muted, fontSize: '11px' }}>{user?.email || 'admin@eventbooking.com'}</span>
              </div>
